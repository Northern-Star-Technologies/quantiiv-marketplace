---
name: query
description: This skill should be used when the user asks about "sales data", "top sellers", "menu items", "product performance", "company locations", "menu groups", "store performance", "weekly sales summary", "weather", "rainfall", "temperature", "labor", "labor cost", "labor hours", "staffing", "scheduling", "sales per labor hour", "SPLH", "labor by job", "labor by role", "labor by position", "turnover", "employee turnover", "headcount", "how many employees", "unique employees", "active employees", "staff count", "shifts", "employee shifts", "shift detail", "who worked", "retention", "attrition", "reviews", "location reviews", "google reviews", "yelp", "star rating", "ratings", "reputation", "public sentiment", "low star", "1-star", "bad reviews", "owner response", "review response", "responded to reviews", "review volume", "review trends", "ottimate", "invoices", "vendor spend", "purchases", "purchasing", "food cost", "cost of goods", "COGS", "invoice detail", "who do we buy from", "gl account", "purchase orders", "accounts payable", "AP", "supplier spend", "fiscal period", "fiscal week", "fiscal year", "fiscal calendar", "P1..P13", "FW1..FW53", "YTD", or any Quantiiv analytics metrics. Also triggered by questions like "what were my top movers", "show me sales trends", "how is this product doing", "list my companies", "compare store performance", "revenue by menu group", "daily sales for [item]", "what was the weather at [location]", "did weather affect sales", "what's my labor cost", "peak labor hours", "labor by location", "am I overstaffed at [hour]", "what's my sales per labor hour", "labor cost by role", "hours by position", "what is my turnover rate", "how many employees do I have", "how many people worked last month", "which employees are still active", "what are people saying about us", "what is my google rating", "show me my worst reviews", "which reviews still need a response", "how is my reputation trending", "how much did we spend on produce", "who are my top vendors", "what did we buy from US Foods last month", "show me invoices over $5000", "what is my food cost by location", "which invoices are unpaid", "what were sales in P3", "fiscal YTD numbers", "this fiscal week", "compare to prior period". When a user references a fiscal period name (e.g. "P3 of FY2026", "fiscal Q1", "fiscal week 12"), resolve it via `client.fiscalCalendar` first to get concrete start/end dates, then pass those dates to the sales/labor/weather endpoints. For "latest"/"current" data, anchor to the daily-updated `latest_available_data_date` from the reporting freshness context — not to the latest week start — and include the current in-progress fiscal week as week-to-date; only default to the last complete week when the user explicitly asks for a complete-week overview. For "this week"/"last week" or any weekly range, align week boundaries to the company's fiscal week start day (`calendar_config.week_start_day`, e.g. Tuesday) — never assume Monday.
allowed-tools: Bash
...
- `latest_available_data_date` is the **actual data-through date**. Use it for all
...
- Keep these concepts separate:
  - `latest_available_data_date` (freshness context) — the newest loaded sales
    date across locations. The freshness anchor: the only value to present as
    data freshness and the end date for "latest" queries.
  - `latest_common_complete_date` / `latest_safe_data_date` (freshness
    context) — the newest date **all** locations are complete through. Not the
    freshness anchor. If it trails `latest_available_data_date`, some
    locations are still catching up — mention that briefly when presenting
    chain-wide totals for those trailing days (check `warnings` /
    `location_scope_summary` for details).
...
- Data normally loads each morning through the prior calendar day, but never
  assume it: if `latest_available_data_date` is older than yesterday, report the
...
- Anchor "current", "latest", "last N days", month-to-date, and relative ranges
  to `latest_available_data_date`, not to today. If a requested range extends past
  it, clip the range to `latest_available_data_date` and tell the user data is only
  available through that date.
- If the freshness call fails or `latest_available_data_date` is null, give a clear
...
- "This week" means the **current in-progress fiscal week** — the week
  containing `latest_available_data_date`. Query it from its fiscal start date
  through `latest_available_data_date` and present it as week-to-date.
...
- **"Latest" / "most recent" defaults to the freshest data available — even if
  ...
  `latest_available_data_date` and include the in-progress week, presented as
...
- For weekly endpoints on a "latest" question, set `week` to the start of the
  **current in-progress fiscal week** (the week containing
  `latest_available_data_date` — resolve via
  `client.fiscalCalendar.resolveFiscalWeek({ reportEndDate: latest_available_data_date })`
  or compute from `week_start_day`) and `to` to `latest_available_data_date`.
...
- Use `"corporate"` as the default location unless the user specifies one
- Cap every date range at `latest_available_data_date` from the reporting freshness context — never assume data exists up to today's calendar date, and never present the weekly anchor (`most_recent_data_date`, or week start + 6) as the data-through date
...
- For "latest"/"most recent" questions, always deliver data through `latest_available_data_date` — including the in-progress fiscal week as week-to-date. Never answer a "latest" question with only the last complete week unless the user explicitly asked for a complete-week overview
- Never report a row or entry count as an employee count. `entryCount` from `getByJob` counts shift rows, so one employee working two shifts looks identical to two employees working one each. For headcount or turnover use `client.labor.listAllShifts()` and count distinct `employeeId`
- Before aggregating employee counts across locations, check `coverage.employeeIdScope`. When it is `"location"` or `"unknown"`, an identifier is only stable within one store, so report per location and note that a chain-wide total would overcount. Only `"company"` allows chain-wide dedup
- Present hires, separations, rehires, and turnover as proxies inferred from clock data, never as HR facts, and state the inactivity threshold, rehire handling, multi-location attribution, and denominator behind any figure. Voluntary vs involuntary turnover cannot be derived at all — say so rather than guessing
- Location Reviews are PUBLIC platform reviews (Google, Yelp, TripAdvisor and others). They are not survey results, customer feedback forms, loyalty data, or Google Analytics — never answer a question about one with the other
- When `coverage.reviewsEnabled` is false the company does not have Location Reviews enabled. Say the capability isn't available for them; NEVER report it as "you have no reviews". An empty window likewise means none in that period, not none ever
- Check `coverage.networks` before calling anything Google reviews — some companies carry nine platforms. `sourceProvider` is only the ingestion vendor, so never attribute a review to it
- A null `rating` is unrated, not zero, and is excluded from `coverage.averageRating`. Use `coverage.withReviewText` as the denominator for text or theme analysis, since many reviews are rating-only
- Omitting both dates on reviews reads only the last 90 days. Pass explicit dates for any other period, and page to the end before quoting a count, average, or trend
- Ottimate is ESR Hospitality purchase/invoice detail at LINE-ITEM grain. Call `client.ottimate.getCoverage()` first — it reports freshness, the earliest data that exists, and the AP fields the feed does not carry
- The Ottimate feed carries NO payment status, payment date, approval status, due date, posting date, cost center, department, category taxonomy, PO number, freight or discount. `invoice_exported_date` means exported to accounting and nothing more. If asked which invoices are unpaid or approved, say the feed does not carry that — never infer it from export state
- An empty Ottimate window is NOT zero spend: the feed is a rolling window with no backfill, so anything before `coverage.first_invoice_date` is missing data
- Group Ottimate vendor spend on `accounting_vendor`, never the printed `vendor_name` (which splits large vendors across rows). Cross-brand vendor rollups are unreliable — say so rather than summing across brands
- Rows with no GL account are ~1.5% of rows carrying ~12% of spend: report an explicit uncategorized bucket, never $0. Negative amounts are genuine credits and must stay in totals
- An Ottimate company IS an ESR brand — scope by `companyId`, there is no brand filter. Invoice totals are sums of landed lines, not source invoice totals
